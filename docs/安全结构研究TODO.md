...
- [x] 对 prefix-zero 二阶段模型增加 q1-only、response-only、q1+response 三视图对照，明确回答文本相对输入攻击文本的增量。
- [ ] 对文本残差脚本增加长跑配置：多 seed、word+char n-gram、分攻击方案报告；预计三攻击全量多 seed 需要 30 分钟到数小时，需写日志和进度。
- [x] 将序贯模拟改成真实预算曲线：初始每条 1 次回答，然后只对 top 候选追加采样，比较固定预算下发现高风险指令的效率。
- [x] 把预算策略区分成两类目标：攻击优化优先平均 future ASR，安全审计优先 prefix-zero 隐蔽高风险召回。

## 2026-07-13 新增：prefix-zero 三视图对照后的任务

- [x] 完成 `response/q1/joint` 三视图轻量实验。结果显示 response 在 AP 和 top recall 上稳定强于 q1，支持回答内容提供输入之外的风险嗅探信号。
- [x] 补多 seed 置信区间，当前完成 5 seed 均值和样本标准差；后续可升级 bootstrap 置信区间。
- [x] 按 OFA、PAIR、DrAttack 分别报告三视图结果，检查 response 优势是否来自某个攻击方案。
- [x] 做同题、同攻击方案内的回答归属打乱。如果打乱后 response 仍强，说明信号更可能是题目、模板或边际分布；如果明显下降，才更支持 q1 和回答之间的具体对应关系。
- [x] 增加危险词、拒绝词、模板残片掩码后的 response 复验，确认当前信号不能被这组显式词面捷径完全解释。
- [x] 将 prefix-zero 二阶段模型接入真实预算模拟：先识别表面安全候选，再决定是否追加采样。
- [x] 运行成本归一化预算曲线：固定总 extra budget，而不是固定 top 20% 候选；脚本为 `scripts/simulate_fixed_extra_budget_asr_discovery.py`。
- [x] 在强掩码基础上做 TF-IDF SVD 轻量语义簇分析，判断剩余信号是否具有低维结构。
- [ ] 用 frozen sentence embedding 复验 SVD 结论，比较 response/q1/joint 和回答归属打乱。
- [ ] 用目标模型 hidden state 复验回答文本分量与白盒伪安全方向、ASR/ALR 的关系。

## 2026-07-13 新增：当前研究口径

- [x] 明确黑盒权限：只能使用 `q1`、模型回答和文本统计特征；白盒 hidden state 只用于机制解释与参照。
- [x] 明确目标模型范围：回答内容与 ASR/ALR 相关性不局限于 Llama，但任一实验必须使用同一目标模型自己的标签。
- [x] 明确研究目标：结构探索和结构利用并行推进，不预设最终结论；优先寻找有效信息与稳定相关性。
- [x] 明确利用目标优先 ASR；ALR、拒绝形式、替代 ASR、伪方向投影作为探索变量。
- [ ] 比较不同 ASR 定义：any-unsafe、判定 hazard、加权 ASR；若重新采样，补多数 unsafe 口径。
- [x] 在固定预算曲线中同时报告攻击优化目标和安全审计目标：前者看 selected future ASR，后者看 prefix-zero 高风险召回。

## 2026-07-13 新增：成分受控信息增益实验

- [x] 实现固定回答成分数据构造：危险回答-only、安全长回答-only、`3` 条安全 + `1` 条危险三种口径。
