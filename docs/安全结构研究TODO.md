...
- [ ] 明确报告：现有经验 ASR 分辨率为 0.05，0.01/0.001 只能作为潜在风险预测或增采目标。
...
- [ ] 针对 ASR 残差本身做高权重词组审计，确认剩余信号是否来自具体危险主题、替代建议、语气姿态或攻击模板残片。
- [ ] 在 HarmBench OFA 上复跑同样残差矩阵，检查数据集迁移是否保持。
- [ ] 做按目标模型分层的校准/误差分析：确认哪些目标模型的 ASR 残差信号最强，是否与基础 ASR 方差有关。
